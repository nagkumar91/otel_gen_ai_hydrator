...
class GenAIUserMessageBody(BaseModel):
    ...
    def validate_role(cls, v):
        ...
        if v != "user":
            raise ValueError("Role must be 'user' for user messages")
        return v
...
class GenAIEventBase(BaseModel):
    ...
    """
    gen_ai_system: Union[GenAISystem, str] = Field(
        ...,
...
class GenAIAssistantMessageBody(BaseModel):
    ...
    @validator('role')
    def validate_role(cls, v):
        """Ensure role is 'assistant' for assistant messages."""
        if v != "assistant":
            raise ValueError("Role must be 'assistant' for assistant messages")
        return v
...
class GenAISystemMessageBody(BaseModel):
    ...
    @validator('role')
    def validate_role(cls, v):
        """Ensure role is 'system' for system messages."""
        if v != "system":
            raise ValueError("Role must be 'system' for system messages")
        return v

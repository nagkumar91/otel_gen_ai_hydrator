...
import json, os, asyncio, logging, sys
...
load_dotenv(dotenv_path=Path(__file__).parent / ".env_sk")
...
# Shipra's App Insights resource ID
app_config = ApplicationInsightsConfig(
    resource_id="/subscriptions/fac34303-435d-4486-8c3f-7094d82a0b60/resourceGroups/rg-naarkalgaihub/providers/Microsoft.Insights/components/appi-naarkalgaihub999971652049",
)

app_insights_connector = ApplicationInsightsConnector(app_config)
span_hydrator = SpanHydrator(app_insights_connector)

invoke_agent_span_id = "03b988e9f388b03c"  # Example span ID for SK Agents
...
# For SK Agents child spans of invoke_agent_span are wrapped in AutoInvocationSpan
child_spans = span_hydrator.get_child_spans(child_spans[0].span_id)
...
async def main():
    ...
    eval_event = create_and_log_evaluation_event(
